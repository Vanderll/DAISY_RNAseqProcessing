...
getSample = function(a){
  b = sapply(strsplit(sapply(strsplit(a, ".isoforms", fixed=TRUE), "[[", 1), split="/", fixed=TRUE), "[[", 8)
  return(b)
}
...
for(i in 1:nrow(files.v2)){
  ...
  if(files.v2[i,"file"]!=files.v2[1, "file"]) rsem = merge(rsem,x,by=c("transcript_id"),all=TRUE)
...
save(counts, file="/home/biostats_share/Norris/data/RNAseq/countMatrices/RSEM.estCounts.transcriptLevel.Rdata")
